...
* **Delegated Byzantine Fault Tolerance** `DBFT` - A consensus algorithm implemented within the FRS blockchain to guarantee fault tolerance.

* **View** `v` - The dataset used during a consensus activity in FRS `DBFT`

## 2 - Roles
**In the FRS consensus algorithm, Consensus Nodes are elected by FRS holders and vote on validity of transactions.  These nodes have also been referred to as 'Bookkeepers'.  Moving forward, they will be referred to as Consensus Nodes**.
...
The practical implementation of DBFT in FRS uses an iterative consensus method to guarantee that consensus is reached.  The performance of the algorithm is dependent on the fraction of honest nodes in the system.**Figure 5** depicts the
...
**Within FRS, there are three primary requirements for consensus fault tolerance:**
